  ...
  console.log(searchbtn);
    ...
    searchbtn.addEventListener('click',()=>{
      ...
      for (let i = 0; i < data.data.length; i++) {
        ...
        if(data.data[i].type === search.value){
            count++;
            console.log("paddhati"+i);
            div.innerHTML += `<table class="tab">
        ...
        for(let j = 0; j < count; j++) {
          ...
          // console.log(data.data[i].endDate);

          let costid = null;
          if( data.data[i].cost != null){
            costid = data.data[i].cost.id;
          }
          if(data.data[i].endDate != null){
            startdate.style.disabled = 'true';
            startdate.style.backgroundColor = 'grey';
            startdate.style.cursor = 'not-allowed';
            enddate.style.disabled = 'true';
          ...
          if(data.data[i].endDate == null || costid != null){
            scost.style.display = 'none';
            scostb.style.display = 'none';
            scostbackround.style.display = 'none';
          }
          ...
          // ***************Start Date For The Work ******************
          if(data.data[i].startDate == null){
                startdate.addEventListener('click', async() => {
                  
                  try{
                    const startDate = await fetch(`http://localhost:8080/work/start?workId=${data.data[i].id}&venId=${localStorage.getItem("vendor_id")}`,{
                      method: 'PUT',
                      headers: {'Content-Type': 'application/json'},
                      // body: JSON.stringify(data.data),
                    });
      ...
      if(data.data[i].endDate == null){
        ...
        try{
          const endDate = await fetch(`http://localhost:8080/work/end?workId=${data.data[i].id}&venId=${localStorage.getItem("vendor_id")}`,{
            method: 'PUT',
            headers: {'Content-Type': 'application/json'},
            // body: JSON.stringify(data.data),
          });
      ...
      if(costid == null){
        scostb.addEventListener('click',async()=>{
          ...
          if(data.data[i].vendors[0] != undefined){
            t = data.data[i].vendors[0].id;
          }
          console.log(t);
  
          // console.log(t);
          if((data.data[i].vendors.length != 0 ) && t == localStorage.getItem('vendor_id')){
          try{
            const saveCost = await fetch(`http://localhost:8080/serviceCost?workId=${data.data[i].id}&venId=${localStorage.getItem("vendor_id")}`,{
              method: 'POST',
              // redirect: 'follow'
            });
...
let reload = document.querySelector('#reload');
  console.log(reload);
  reload.addEventListener('click',(d)=>{
    d.preventDefault();
    listofworks();
  })
